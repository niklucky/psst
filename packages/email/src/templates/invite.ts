...
export function inviteEmail({ orgName, inviterEmail, inviteUrl, role }: InviteEmailParams): EmailContent {
  const subject = `${inviterEmail} invited you to join ${orgName} on Psst`;

  const html = renderLayout(`
    <p style="margin:0 0 16px;">Hi,</p>
    <p style="margin:0 0 16px;"><strong>${escapeHtml(inviterEmail)}</strong> invited you to join <strong>${escapeHtml(orgName)}</strong> on Psst as a <strong>${escapeHtml(role)}</strong>.</p>
    <p style="margin:0 0 24px;">Psst is an end-to-end encrypted secrets manager — your data is encrypted on your device before it ever reaches the server.</p>
  ...
  const text = `${inviterEmail} invited you to join ${orgName} on Psst as a ${role}.
