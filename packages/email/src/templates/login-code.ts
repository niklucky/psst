...
export function loginCodeEmail({ code }: LoginCodeEmailParams): EmailContent {
  const subject = `${code} is your Psst sign-in code`;

  const html = renderLayout(`
    <p style="margin:0 0 16px;">We noticed a sign-in to your Psst account from a new device or browser.</p>
  ...
  const text = `We noticed a sign-in to your Psst account from a new device or browser.
